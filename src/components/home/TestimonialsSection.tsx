    ...
    <section className="relative py-16 md:py-24 bg-white overflow-hidden">
      <div className="container relative z-10 px-4 md:px-6">
        <AnimatedSection className="text-center mb-10 md:mb-14">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-charcoal mb-4">What Our Clients Are Saying</h2>
          <p className="text-base md:text-lg text-mountain-slate max-w-2xl mx-auto">Real 5-star reviews from Colorado homeowners</p>
        </AnimatedSection>
        ...
        <div className="md:hidden">
          <div className="flex gap-4 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-4 -mx-4 px-4">
            {testimonials.map((testimonial, index) => (
              <div key={index} className="flex-shrink-0 w-[85%] snap-center">
                <div className="relative bg-white rounded-2xl p-6 border border-border shadow-md overflow-hidden transition-all duration-300 h-full">
          ...
          <div className="flex justify-center gap-1.5 mt-4">
            {testimonials.map((_, i) => (
              <div key={i} className="w-1.5 h-1.5 rounded-full bg-charcoal/20" />
            ))}
          </div>
        </div>
        ...
        <StaggerContainer className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {testimonials.map((testimonial, index) => (
            <StaggerItem key={index}>
              <div className="relative bg-white rounded-2xl p-6 md:p-8 border border-border shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-primary/20 h-full">
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary" />
                <div className="flex items-center gap-1 mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="w-5 h-5 text-gold fill-gold" />
        ...
        <AnimatedSection delay={0.3} className="mt-10">
          <div className="flex flex-wrap items-center justify-center gap-3 text-mountain-slate text-sm">
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-4 h-4 text-gold fill-gold" />
              ))}
            </div>
            <span className="font-semibold text-charcoal">5.0 Average Rating</span>
            <span>•</span>
